...
function get_package_relpath_in_registry(; package_name::String,
    ...
    matching_package_indices = findall(getindex.(all_package_names_and_paths, 1) .== package_name)
    num_indices = length(matching_package_indices)
    (num_indices == 0) && throw(ErrorException("no package found with the name $(package_name)"))
    (num_indices != 1) && throw(ErrorException("multiple ($(num_indices)) packages found with the name $(package_name)"))
