function pull_request_build(::NewPackage,
    ...
    if is_open(pr)
        if pr_author_login in authorized_authors
            ...
            my_retry(() -> GitHub.create_status(registry, current_pr_head_commit_sha; auth=auth, params=Dict("state" => "pending", "context" => "automerge/decision")))
            ...
            if all(newp_g0through5)
                ...
                if newp_g6and7
                    ...
                    my_retry(() -> GitHub.create_status(registry, current_pr_head_commit_sha; auth=auth, params=Dict("state" => "success", "context" => "automerge/decision", "description" => "$(current_pr_head_commit_sha)")))
                    return nothing
                else
                    ...
                    my_retry(() -> GitHub.create_status(registry, current_pr_head_commit_sha; auth=auth, params=Dict("state" => "failure", "context" => "automerge/decision")))
