...
Base.@kwdef struct TestProps
    ...
    # number of backticks in fence, or nothing if not checking:
    fence_count::Union{Int,Nothing} = nothing
end
...
function check_properties(result, props::TestProps)
    ...
    # Check fence count if specified
    if props.fence_count !== nothing
        if !uses_fences(result, props.fence_count)
            return false, "Expected $(props.fence_count) backticks in fence"
        end
    end
...
@testset "format_diff_stats" begin
    ...
    test_cases = [
        ...
        (name = "13 lines → details",
         ...
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_details=true, has_stat=true)),
        ...
        (name = "Long stat → shortstat",
         ...
         props = TestProps(has_details=true, has_shortstat=true)),
        ...
        (name = "50,001 chars → omit",
         ...
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_omitted_msg=true, has_stat=true)),
        ...
        (name = "Invalid UTF-8 → message",
         ...
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_utf8_msg=true, has_stat=true)),
        ...
        (name = "Triple backticks → ````",
         ...
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_inline_diff=true, fence_count=4)),

        (name = "Four backticks → `````",
         full_diff = "````code````",
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_inline_diff=true, fence_count=5)),

        (name = "Fence escaping in details",
         full_diff = join(["line $i```" for i in 1:20], "\n"),
         stat = default_stat,
         shortstat = default_shortstat,
         props = TestProps(has_details=true, has_stat=true, fence_count=4)),
        ...
        (name = "Invalid UTF-8 with long stat → shortstat",
         ...
         props = TestProps(has_utf8_msg=true, has_shortstat=true)),
    ]
