...
function pr_comment_is_blocking(c::GitHub.Comment)
    ...
    lower_body = lowercase(body(c))
    not_blocking =
        occursin("[noblock]", lower_body) ||
        occursin("[no block]", lower_body) ||
        occursin("[merge approved]", lower_body)
    return !not_blocking
end
...
function cron_or_api_build(
    ...
    if blocked
        ...
        return nothing
    elseif has_label(pr.labels, BLOCKED_LABEL) && !registry_config.read_only
    ...
    if pr_type == :NewPackage # it is a new package
        ...
        if merge_config.merge_new_packages
            @info(
                string(
                    "Pull request: $(pr_number). ",
                    "Type: $(pr_type). ",
        ...
        if merge_config.merge_new_versions
            @info(
                string(
                    "Pull request: $(pr_number). ",
                    "Type: $(pr_type). ",
